...
const TEAM_ID = '0'; // Default team ID

async function runTest() {
    // 1. Create a board
    console.log('Creating board...');
    const createRes = await fetch(`${BASE_URL}/api/boards`, {
        method: 'POST',
    ...
    const toggleRes1 = await fetch(`${BASE_URL}/api/boards/${board.id}/toggle-favorite`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'}
    });
    ...
    const listRes = await fetch(`${BASE_URL}/api/teams/${TEAM_ID}/boards`);
    ...
    const toggleRes2 = await fetch(`${BASE_URL}/api/boards/${board.id}/toggle-favorite`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'}
    });
    ...
    await fetch(`${BASE_URL}/api/boards/${board.id}`, {method: 'DELETE'});
    console.log('Done.');
}
